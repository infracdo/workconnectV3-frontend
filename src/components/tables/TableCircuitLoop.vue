  ...
  >
    ...
    <template v-slot:body="props">
      ...
      <q-tr
        ...
        class="clickable"
        :class="['clickable', siteIds.includes(props.row.site_id) ? 'highlight-row' : '']"
      >
        ...
        <q-td key="public_ip_primary" :props="props">
          {{ props.row.discovered_ip_vpn_1 }}
        </q-td>
        <q-td key="public_ip_backup" :props="props">
          {{ props.row.discovered_ip_vpn_2 }}
        </q-td>
...
import { ICircuitLoops, IdetailsListData } from "src/api/types";
...
import { insertWorkconnectLog, getDetailsList } from "src/api/api";
import { hasPermission } from "src/utils/helper";
const props = defineProps<{
  rows: ICircuitLoops[] | IdetailsListData[];
  title: string;
  closeBtn: boolean;
  cardTitle?: string;
  refresh?: () => void;
...
const getDownStores = async () => {
  siteIds.value = (await getDetailsList("down_store")).map((s: any) => s.site_id);
};
...
onMounted(() => {
  columns.value = hasPermission([
    ...
    ? store.$state.storeCircuitLoop
    : store.$state.storeCircuitLoopPSC!;
});
  getDownStores();
</script>
